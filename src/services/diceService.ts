...
// Проверка с характеристикой и навыком (по правилам Гарри Поттер)
export function rollCheck(
  characteristic: number,
  skill: number,
  modifier: number = 0,
  difficulty: number = 12
): DiceRoll & { success: boolean; degree: 'critical' | 'success' | 'failure' | 'fumble' } {
...
export function addToHistory(roll: DiceRoll): void {
  ...
  try {
    ...
    console.error('Error saving roll history:', e);
  }
}
...
// Генерация красивого текста для броска
export function formatRollText(roll: DiceRoll): string {
  ...
  if (roll.modifier !== 0) {
    text += ` ${roll.modifier > 0 ? '+' : ''}${roll.modifier}`;
  }
