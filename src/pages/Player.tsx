...
import { LogOut, BookOpen, Sparkles, FlaskConical, Dice5, Backpack, Settings, Users, User, Plus, FileText } from 'lucide-react';
    ...
    <div className="flex flex-col h-screen bg-soft-ivory">
      ...
      <main className="flex-1 flex gap-2 p-2 overflow-hidden">
        ...
        <div className="w-80 flex flex-col">
          <Chat messages={messages} onSendMessage={sendMessage} currentUserName={playerName || 'Игрок'} className="flex-1" maxHeight="calc(100vh - 200px)" />
        </div>
      </main>

      <footer className="bg-soft-ivory/80 backdrop-blur-sm border-t border-caramel/20 p-2 flex justify-around items-center">
        <button onClick={() => { if (myCharacter) { setSelectedCharacter(myCharacter); setShowCharacterSheet(true); } }} className="flex flex-col items-center text-walnut/60 hover:text-caramel transition-colors">
          <User className="w-5 h-5" />
          <span className="text-[10px] font-medium mt-0.5">Персонаж</span>
        </button>
        <button onClick={() => setShowRules(true)} className="flex flex-col items-center text-walnut/60 hover:text-caramel transition-colors">
        ...
        <button onClick={() => setShowDiceRoller(true)} className="flex flex-col items-center text-caramel relative">
        ...
        <button onClick={() => setShowSessionLog(true)} className="flex flex-col items-center text-walnut/60 hover:text-caramel transition-colors">
        ...
        <div className="absolute right-4 top-16 bg-soft-ivory rounded-xl shadow-xl border border-caramel/20 p-4 min-w-64 z-10">
          ...
          {characters.length === 0 ? (
            ...
            <div className="space-y-2">
              {characters.map(char => (
                <button key={char.id} onClick={() => handleSelectCharacter(char)} className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${myCharacter?.id === char.id ? 'bg-caramel/10 border border-caramel/30' : 'hover:bg-vanilla-cream/50'}`}>
      ...
      {showCharacterSheet && selectedCharacter && (
        <CharacterSheet character={selectedCharacter} onClose={() => { setShowCharacterSheet(false); setShowCharacterList(false); }} readOnly={false} />
      )}

      {showDiceRoller && (
        <DiceRoller onRoll={handleRoll} onClose={() => setShowDiceRoller(false)} userId={userId} userName={playerName || 'Игрок'} />
      )}
