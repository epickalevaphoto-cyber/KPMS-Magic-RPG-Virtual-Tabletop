...
interface LogEntry {
  id: string;
  timestamp: number;
  type: 'message' | 'roll' | 'system' | 'combat' | 'discovery';
  text: string;
  userId: string;
  userName: string;
}
      ...
      <div className="bg-soft-ivory rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        <div className="flex justify-between items-center px-6 py-4 border-b border-caramel/20">
          ...
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="sm" onClick={exportLogs} className="text-walnut/60 hover:text-caramel"><Download className="w-4 h-4" /></Button>
            <Button variant="ghost" size="sm" onClick={clearLogs} className="text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></Button>
              ...
              <div key={log.id} className={`flex items-start space-x-3 p-3 rounded-lg border ${getTypeColor(log.type)}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-xs text-walnut/40">{new Date(log.timestamp).toLocaleTimeString()}</span>
                  </div>
