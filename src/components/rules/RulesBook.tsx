...
import { Search, X, BookOpen, Wand2, FlaskConical, Sword, Shield, Users } from 'lucide-react';
      ...
      <div className="bg-soft-ivory rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-3 border-b border-caramel/10">
          <div className="relative">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Поиск по правилам..." className="w-full pl-10 pr-4 py-2 rounded-lg border border-caramel/30 focus:border-caramel focus:outline-none transition-colors bg-white" />
          </div>
        </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="w-64 border-r border-caramel/20 overflow-y-auto p-3 space-y-1">
            {filteredSections.map(section => (
              <button key={section.id} onClick={() => setSelectedSection(section)} className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors flex items-center space-x-2 ${selectedSection?.id === section.id ? 'bg-caramel/10 text-caramel' : 'text-walnut/70 hover:bg-krem'}`}>
                <span className="text-walnut/40">{section.icon}</span>
                <span>{section.title}</span>
              </button>
            ))}
            {filteredSections.length === 0 && <p className="text-sm text-walnut/40 text-center py-4">Ничего не найдено</p>}
          </div>
            ...
            {selectedSection ? (
              <div className="prose prose-sm max-w-none">
                {selectedSection.content.split('\n').map((line, i) => {
                  if (line.startsWith('# ')) return <h1 key={i} className="font-serif text-2xl font-bold text-dark-chocolate mt-0 mb-4">{line.slice(2)}</h1>;
                  if (line.startsWith('## ')) return <h2 key={i} className="font-serif text-xl font-semibold text-dark-chocolate mt-6 mb-2">{line.slice(3)}</h2>;
                  if (line.startsWith('### ')) return <h3 key={i} className="font-serif text-lg font-semibold text-dark-chocolate mt-4 mb-1">{line.slice(4)}</h3>;
                  if (line.startsWith('- ')) return <li key={i} className="text-walnut/80 ml-4">{line.slice(2)}</li>;
