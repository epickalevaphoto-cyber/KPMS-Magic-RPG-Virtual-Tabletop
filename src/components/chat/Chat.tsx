...
const Chat = ({ 
  ...
  const getMessageStyle = (message: ChatMessage) => {
    switch (message.type) {
      ...
      case 'system': return 'bg-walnut/5 border-walnut/10 text-walnut/60 italic';
  ...
  return (
    <div className={`flex flex-col bg-white/40 backdrop-blur-sm rounded-xl border border-caramel/20 overflow-hidden ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-1.5" style={{ maxHeight }}>
        {messages.length === 0 ? (
          <p className="text-xs text-walnut/40 text-center py-4">Нет сообщений</p>
        ) : (
          messages.map((message) => (
            <div key={message.id} className={`flex items-start space-x-2 p-2 rounded-lg border ${getMessageStyle(message)}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-sm text-dark-chocolate/80 break-words">{message.text}</div>
              </div>
            </div>
          ))
        )}
      ...
      <div className="border-t border-caramel/20 p-3 bg-soft-ivory/50">
        <div className="flex space-x-2">
          ...
          />
          <Button variant="primary" size="sm" onClick={handleSend} disabled={!inputText.trim()} className="px-3">
