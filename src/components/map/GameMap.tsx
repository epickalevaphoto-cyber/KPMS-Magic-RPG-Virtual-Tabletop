...
const GameMap = ({ roomCode, isMaster = false, tokens = [], onTokenMove }: GameMapProps) => {
  ...
  useEffect(() => {
    ...
    if (savedTokens) setLocalTokens(JSON.parse(savedTokens));
  }, [roomCode]);
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const updated = localTokens.map(t => t.id === dragToken ? { ...t, x: clampedX, y: clampedY } : t);
    setLocalTokens(updated);
    localStorage.setItem(`tokens_${roomCode}`, JSON.stringify(updated));
    if (onTokenMove) onTokenMove(dragToken, clampedX, clampedY);
  };

  const handleMouseUp = () => setDragToken(null);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 p-2 bg-soft-ivory/80 rounded-t-xl border border-caramel/20 flex-wrap">
        {isMaster && (
          <>
            <Button variant="secondary" size="sm" onClick={() => fileInputRef.current?.click()}><Upload className="w-4 h-4 mr-1" /> Карта</Button>
            <input ref={fileInputRef} type="file" accept="image/*" onChange={handleMapUpload} className="hidden" />
            <Button variant="secondary" size="sm" onClick={() => addToken('Персонаж')}><Plus className="w-4 h-4 mr-1" /> Токен</Button>
            <button onClick={() => setFogOfWar(!fogOfWar)} className={`p-2 rounded-lg transition-colors ${fogOfWar ? 'bg-caramel/20 text-caramel' : 'text-walnut/60 hover:bg-caramel/10'}`} title="Туман войны">
              {fogOfWar ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
            </button>
            <Button variant="ghost" size="sm" onClick={clearMap} className="text-red-400 hover:text-red-600"><X className="w-4 h-4" /></Button>
          </>
        )}
        <div className="flex-1"></div>
        <div className="flex items-center gap-1">
          <button onClick={() => setZoom(z => Math.max(0.5, z - 0.1))} className="p-1 text-walnut/60 hover:text-caramel"><ZoomOut className="w-4 h-4" /></button>
          <span className="text-xs text-walnut/60 w-12 text-center">{Math.round(zoom * 100)}%</span>
          <button onClick={() => setZoom(z => Math.min(2, z + 0.1))} className="p-1 text-walnut/60 hover:text-caramel"><ZoomIn className="w-4 h-4" /></button>
        </div>
      </div>

      <div ref={mapContainerRef} className="flex-1 relative bg-walnut/10 rounded-b-xl border border-caramel/20 overflow-hidden" style={{ minHeight: '300px' }} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp}>
        {mapImage ? (
          <div className="relative w-full h-full" style={{ transform: `scale(${zoom})`, transformOrigin: 'center' }}>
            ...
            {localTokens.map(token => (
              <div key={token.id} className={`absolute rounded-full cursor-${isMaster ? 'grab' : 'default'} flex items-center justify-center text-white text-xs font-bold shadow-lg transition-transform hover:scale-110`} style={{ left: `${token.x}%`, top: `${token.y}%`, width: `${token.size}px`, height: `${token.size}px`, backgroundColor: token.color, transform: 'translate(-50%, -50%)', zIndex: selectedToken === token.id ? 10 : 1 }} onMouseDown={(e) => handleMouseDown(e, token.id)} onClick={() => setSelectedToken(token.id === selectedToken ? null : token.id)}>
                {token.name[0]?.toUpperCase() || '?'}
              </div>
            ))}
            {fogOfWar && <div className="absolute inset-0 bg-black/70 flex items-center justify-center pointer-events-none"><p className="text-white/30 text-sm">🌫️ Туман войны</p></div>}
          </div>
        ) : (
        ...
        <div className="p-2 bg-soft-ivory/80 rounded-b-xl border border-caramel/20 flex flex-wrap gap-1">
          {localTokens.map(t => (
            <span key={t.id} className="flex items-center gap-1 px-2 py-1 bg-white rounded-lg text-xs shadow-sm">
              ...
              {t.name}
              <button onClick={() => removeToken(t.id)} className="text-red-400 hover:text-red-600"><X className="w-3 h-3" /></button>
            </span>
          ))}
        </div>
      )}
